...
MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes))
            {
                ...
                mahang = DataGridViewChitiet.CurrentRow.Cells["MaNoiThat"].Value.ToString();
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                string sql = "SELECT MaNoiThat,SoLuong FROM ChiTietDonDatHang WHERE SoDDH = N'" + txtSoDDH.Text + "'";
                DataTable tblHang = DAO.GetDataToTable(sql);
                for (int hang = 0; hang <= tblHang.Rows.Count - 1; hang++)
                {
                    // Cập nhật lại số lượng cho các mặt hàng
                    sl = Convert.ToDouble(DAO.GetFieldValues("SELECT SoLuong FROM DMNoiThat WHERE MaNoiThat = N'" + tblHang.Rows[hang][0].ToString() + "'"));
                    slxoa = Convert.ToDouble(tblHang.Rows[hang][1].ToString());
                    slcon = sl + slxoa;
                    sql = "UPDATE DMNoiThat SET SoLuong =" + slcon + " WHERE MaNoiThat= N'" + tblHang.Rows[hang][0].ToString() + "'";
                    DAO.RunSQL(sql);
                }

                //Xóa chi tiết hóa đơn
                sql = "DELETE ChiTietDonDatHang WHERE SoDDH=N'" + txtSoDDH.Text + "'";
                DAO.RunSqlDel(sql);

                //Xóa hóa đơn
                sql = "DELETE DonDatHang WHERE SoDonDatHang=N'" + txtSoDDH.Text + "'";
                DAO.RunSqlDel(sql);
                ResetValues();
                Load_DataGridViewChitiet();
                btnXoa.Enabled = false;
...
MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        ...
        private void cmbMaNoiThat_TextChanged(object sender, EventArgs e)
        {
            ...
            txtThanhTien.Text = DAO.GetFieldValues(str);
            str = "SELECT MaNV FROM ChiTietDonDatHang  as a inner join DonDatHang as b on a.SoDDH=b.SoDDH WHERE MaNoiThat = N'" + cmbMaNoiThat.SelectedValue + "'";
            cmbMaNV.Text = DAO.GetFieldValues(str);
            str = "SELECT TenNV FROM NhanVien as a inner join DonDatHang as b on a.MaNV=b.MaNV inner join ChiTietDonDatHang as c on c.SoDDH=b.SoDDH WHERE MaNoiThat = N'" + cmbMaNoiThat.SelectedValue + "'";
            txtTenNV.Text = DAO.GetFieldValues(str);
            str = "SELECT MaKhach FROM DonDatHang as b inner join ChiTietDonDatHang as c on c.SoDDH=b.SoDDH WHERE MaNoiThat = N'" + cmbMaNoiThat.SelectedValue + "'";
            cmbMaKH.Text = DAO.GetFieldValues(str);
            str = "SELECT TenKhach FROM KhachHang as a inner join DonDatHang as b on a.MaKhach=b.MaKhach inner join ChiTietDonDatHang as c on c.SoDDH=b.SoDDH WHERE MaNoiThat = N'" + cmbMaNoiThat.SelectedValue + "'";
            txtTenKH.Text = DAO.GetFieldValues(str);
            str = "SELECT DiaChi FROM KhachHang as a inner join DonDatHang as b on a.MaKhach=b.MaKhach inner join ChiTietDonDatHang as c on c.SoDDH=b.SoDDH WHERE MaNoiThat = N'" + cmbMaNoiThat.SelectedValue + "'";
            txtDiaChi.Text = DAO.GetFieldValues(str);
            str = "SELECT DienThoai FROM KhachHang as a inner join DonDatHang as b on a.MaKhach=b.MaKhach inner join ChiTietDonDatHang as c on c.SoDDH=b.SoDDH WHERE MaNoiThat = N'" + cmbMaNoiThat.SelectedValue + "'";
            txtDienThoai.Text = DAO.GetFieldValues(str);
            str = "SELECT TongTien FROM DonDatHang as b inner join ChiTietDonDatHang as c on c.SoDDH=b.SoDDH WHERE MaNoiThat = N'" + cmbMaNoiThat.SelectedValue + "'";
            txtTongTien.Text = DAO.GetFieldValues(str);*/
        }
